    ...
    with st.spinner("Analyzing match control..."):
        ...
        with tab_goals_against:
            ...
            if has_team_matches_goals and selected_team:
                ...
                # Analysis for all teams in database for comparison
                st.subheader("Vergelijking met alle teams in database")
                with st.spinner("Analyseren van alle wedstrijden in database..."):
                    all_teams_stats = {}  # team_name -> {total_goals_against, total_xg_before, total_net_control_before, total_shots_before}
                    ...
                    for match_info in files_info:
                        ...
                        for goal in goals:
                            ...
                            # Determine which team conceded (the other team)
                            if goal_team == home_team_match:
                                conceding_team = away_team_match
                                scoring_team = home_team_match
                            elif goal_team == away_team_match:
                                conceding_team = home_team_match
                                scoring_team = away_team_match
                            else:
                                continue
                            ...
                            # Calculate metrics in the 5 minutes before
                            xg_against_before = sum_xg(scoring_team, before_start, before_end)
                            control_scoring_before = sum_control_points(scoring_team, before_start, before_end)
                            control_conceding_before = sum_control_points(conceding_team, before_start, before_end)
                            net_control_before = control_scoring_before - control_conceding_before
                            shots_against_before = count_shots(scoring_team, before_start, before_end)

                            # Accumulate stats for the conceding team
                            if conceding_team not in all_teams_stats:
                                all_teams_stats[conceding_team] = {
                                    'total_goals_against': 0,
                                    'total_xg_before': 0.0,
                                    'total_net_control_before': 0.0,
                                    'total_shots_before': 0
                                }

                            all_teams_stats[conceding_team]['total_goals_against'] += 1
                            all_teams_stats[conceding_team]['total_xg_before'] += xg_against_before
                            all_teams_stats[conceding_team]['total_net_control_before'] += net_control_before
                            all_teams_stats[conceding_team]['total_shots_before'] += shots_against_before

                    # Create comparison table
                    if all_teams_stats:
                        comparison_rows = []
                        for team_name, stats in sorted(all_teams_stats.items(), key=lambda x: x[0].lower()):
                            num_goals = stats['total_goals_against']
                            if num_goals > 0:
                                avg_xg = stats['total_xg_before'] / num_goals
                                avg_net_control = stats['total_net_control_before'] / num_goals
                                avg_shots = stats['total_shots_before'] / num_goals
                                
                                comparison_rows.append({
                                    "Team": team_name,
                                    "Aantal tegendoelpunten": num_goals,
                                    "Gem. xG tegen (5 min voor)": round(avg_xg, 2),
                                    "Gem. net controle (5 min voor)": round(avg_net_control, 2),
                                    "Gem. schoten tegen (5 min voor)": round(avg_shots, 2)
                                })

                        if comparison_rows:
                            df_comparison = pd.DataFrame(comparison_rows)
                            
                            # Highlight selected team if available
                            if selected_team:
                                st.write(f"**Geselecteerd team:** {selected_team}")
                            ...
                            st.dataframe(
                                ...
                                column_config={
                                    ...
                                    "Aantal tegendoelpunten": st.column_config.NumberColumn("Aantal tegendoelpunten", width="small"),
                                    "Gem. xG tegen (5 min voor)": st.column_config.NumberColumn("Gem. xG tegen (5 min voor)", width="small", format="%.2f"),
                                    "Gem. net controle (5 min voor)": st.column_config.NumberColumn("Gem. net controle (5 min voor)", width="small", format="%.2f"),
                                    "Gem. schoten tegen (5 min voor)": st.column_config.NumberColumn("Gem. schoten tegen (5 min voor)", width="small", format="%.2f")
                                }
                            )
                    else:
                        st.info("Geen data gevonden voor vergelijking.")
